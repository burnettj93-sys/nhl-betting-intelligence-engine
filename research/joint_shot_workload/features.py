"""
PIT-safe access over the joint corpus itself -- used for the player-share
model (Part 8/9), which needs matched (player SOG, team SOG) pairs from
the SAME games, already linked by build_joint_corpus.py.
"""
...
CORPUS_PATH = Path(__file__).resolve().parent / "joint_shot_workload.jsonl"


def load_joint_corpus(path: str | Path = CORPUS_PATH) -> list[dict]:
    rows = []
    with open(path) as f:
        for line in f:
            line = line.strip()
