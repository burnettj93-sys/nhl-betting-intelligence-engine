...
## Update — 2026-09-25 (Live Run Reliability block)

* **Missed-slot catch-up (no new job).** launchd never replays a calendar slot missed while the Mac was off, so one missed 07:00 sync stalled the whole morning chain. `operational.nhl_sync --mode=pregame` (the existing 30-minute NHL job) now runs `operational/morning_catchup.py` first. It does nothing on a normal day. If a stage's slot **+ 30 min** has passed today with no recorded success since the slot, it runs the missing stages **in dependency order, once**, through the same entry points launchd uses: `sync_daily.py` (07:00) → `settle_daily_observations` (07:15) → `daily_postmortem` (07:30); `backup_databases` (07:45) is independent. Bounds: lock file, ≤ 2 attempts per stage per local day, ≥ 30 min between attempts (the NHL API is not hammered), hard subprocess timeouts, a stage never runs while its upstream stage is unrecovered, STANDBY machines skip, never raises, attempt recorded *before* running, reason `CATCHUP_AFTER_MISSED_SLOT`. State: `operational/runtime/morning_catchup_state.json`. Late-boot example: Mac starts 08:30 → the next 30-minute NHL refresh (≤ 09:00) recovers sync, settlement, post-mortem and backup the same day. The normal 07:00–07:45 schedule is unchanged.
* **`moneyline-pregame` heartbeat + missed-window detection** (see `docs/LIVE_MONEYLINE_CERTIFICATION.md`).
* Inventory unchanged: **11 jobs**, `python3 -m operational.scheduler_audit` OK.
* **Test/production isolation:** tests can no longer resolve production state paths (`operational/state_paths.py`); tests can no longer launch the real cloud publisher or make a real Odds API request. See the blocker matrix (Live Run Reliability).
